...
public class FOWRevealer : MonoBehaviour
{
	...
	void LateUpdate ()
	{
		if (isActive)
		{
			if (lineOfSightCheck != FOWSystem.LOSChecks.OnlyOnce) mRevealer.mCachedBuffer = null;

			mRevealer.mPos = mTrans.position;
			mRevealer.mInner = range.x;
			mRevealer.mOuter = range.y;
			mRevealer.mLos = lineOfSightCheck;
			mRevealer.mIsActive = true;
		}
		else
		{
			mRevealer.mIsActive = false;
			mRevealer.mCachedBuffer = null;
		}
	}
	...
	public void Rebuild () { mRevealer.mCachedBuffer = null; }
}